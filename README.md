...
- **Captive Portal** — a pure `java.io` server (port 1080) that completes the SOCKS5 handshake, intercepts unauthenticated clients, and redirects captive-portal probes to a voucher login page so the OS shows its native "Sign in to network" / "Login to network" flow on the Wi-Fi list.
...
3. **Client connects** — traffic arrives at the local proxy as SOCKS5. Unauthenticated clients receive a SOCKS5 success reply so their HTTP payload surfaces; captive-portal probes (connectivity checks) are answered with a 302 redirect to the voucher login page served on the group owner's address, which triggers the OS-native "Sign in to network" notification and "Login to network" entry in Wi-Fi settings. The login form itself is also reachable directly at the group owner's address on the proxy port.
